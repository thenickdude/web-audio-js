...
} = require('../../src/constants/AudioParamEvent');
const {
...
describe('utils/AudioParamUtils.computeValueAtTime(timeline: object[], time: number, defaultValue: number): number', () => {
  it('works', () => {
    ...
    assert(AudioParamUtils.computeValueAtTime(timeline, 0.0, 0) === 0.2);
    assert(AudioParamUtils.computeValueAtTime(timeline, 0.05, 0) === 0.2);
    assert(AudioParamUtils.computeValueAtTime(timeline, 0.1, 0) === 0.3);
    assert(AudioParamUtils.computeValueAtTime(timeline, 0.15, 0) === 0.3);
    assert(AudioParamUtils.computeValueAtTime(timeline, 0.2, 0) === 0.4);
    assert(AudioParamUtils.computeValueAtTime(timeline, 0.25, 0) === 0.7);
    assert(AudioParamUtils.computeValueAtTime(timeline, 0.3, 0) === 1.0);
